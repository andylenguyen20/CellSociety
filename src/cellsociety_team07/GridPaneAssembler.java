...
public class GridPaneAssembler extends Visualizer {
	...
	public void assembleGridPane(GridPane g, MenuCreator m) {
		...
		g.add(m.makeButton(myResources_C), 1, 3);
		g.add(new Label(m.getResources(myResources_S, "LabelCommand")), 0, 0);
		g.add(m.getSimulationMenu(myResources_S), 1, 0);
		g.add(new Label(m.getResources(myResources_C, "LabelCommand")), 2, 0);
		g.add(m.getCommandsBox(myResources_C), 3, 0);
	}
